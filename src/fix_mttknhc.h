...
FixStyle(mttknhc,FixMTTKNHC)

#else

#ifndef LMP_FIX_MTTKNHC_H
#define LMP_FIX_MTTKNHC_H

#include "fix.h"

namespace LAMMPS_NS {

class FixMTTKNHC : public Fix {
 public:
  FixMTTKNHC(class LAMMPS *, int, char **);
  virtual ~FixMTTKNHC();
  virtual void init();
  int setmask();
 ...
 protected:
  ...
  int vol_constraint;              // 1 if using nvsigmaa=0t ensemble
  int pdim;                        // number of barostatted dims
  double ndof_baro;                // number of barostat DOFs
